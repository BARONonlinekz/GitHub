jQuery(document).ready(function($) {
    ...
    var lim;
	var checkText=true;
	var text_anim='Ответьте на несколько вопросов по разработке платформы дистанционного обучения согласно Вашим требованиям!';
    ...
    function animateOnNew() {
        if(!onetrue){
            ...
            function animateOnN(activeTxt) {
                ...
                if (activeTxt == 5) {
                    activeTxt = 1;
                    animateOnN(activeTxt + 1);
                }
                if (onceN && activeTxt == 1) {
                    return;
                }
                $('.after-img[data-id=' + activeTxt + ']').addClass('active');
                $('.after-img[data-id=' + activeTxt + ']').siblings('.fact').addClass('active');
                setTimeout(function() {
                    $('.after-img[data-id=' + activeTxt + ']').removeClass('active');
                    $('.after-img[data-id=' + activeTxt + ']').siblings('.fact').removeClass('active');
                    if (!onceN && activeTxt == 1) {
                        return;
                    }
                    animateOnN(activeTxt + 1);
    ...
    function animateOn1() {
        ...
        $("#lp-luch1 img").css("-webkit-animation", "leaves 2s ease-in-out infinite alternate", "animation", "leaves 2s ease-in-out infinite alternate");
        $('.line-5, .line-6').addClass('active').animate({
            opacity: '1'
        }, 500, function() {
            $('.line-3, .line-4').addClass('active').animate({
                opacity: '1'
            }, 500, function() {
                $('.line-2, .line-1').addClass('active').animate({
                    opacity: '1'
                }, 500, function() {
                    $('img.o-anim').addClass('active').animate({
                        opacity: '1'
                    }, 500, function() {
                        $('.types-item-img2 img').addClass('active animated fadeInUp').animate({
                            opacity: '1'
                        }, 500, function() {
                            $(".types-item").hover(function() {
                                $(this).find('.types-item-img2').find('img').addClass('margg');
                                $(this).find('.bottom-border').addClass('active');
                            }, function() {
                                $(this).find('.types-item-img2').find('img').removeClass('margg');
                                $(this).find('.bottom-border').removeClass('active');
                            });
                        });
                    });
                });
    ...
    function animateOn2() {
        ...
        $(".items-box").hover(function() {
            $(this).find('.items-img').find('.animated-default').find('img').addClass('animated bounceIn');
        }, function() {
            $(this).find('.items-img').find('.animated-default').find('img').removeClass('animated bounceIn');
        });
        setTimeout(function() {
            if(onetrue2 == false){
                var onceB = false;
                ...
                function animateOnB(activeTxt) {
                    ...
                    if (activeTxt == 10) {
                        activeTxt = 1;
                        animateOnB(activeTxt + 1);
                    }
                    if (onceB && activeTxt == 1) {
                        return;
                    }
                    $('.items-img[data-id=' + activeTxt + ']').find('.animated-default').addClass('active');
                    $('.items-img[data-id=' + activeTxt + ']').find('.animated-default').find('img').addClass('active');
                    $('.items-img[data-id=' + activeTxt + ']').parents('.items-box').find('.items-text').find('p').addClass('animated pulse').css('color', '#00ff00');
                    setTimeout(function() {
                        $('.items-img[data-id=' + activeTxt + ']').find('.animated-default').find('img').removeClass('active');
                        $('.items-img[data-id=' + activeTxt + ']').find('.animated-default').removeClass('active');
                        $('.items-img[data-id=' + activeTxt + ']').parents('.items-box').find('.items-text').find('p').removeClass('animated pulse').css('color', 'white');
                        if (!onceB && activeTxt == 1) {
                            return;
                        }
                        animateOnB(activeTxt + 1);
