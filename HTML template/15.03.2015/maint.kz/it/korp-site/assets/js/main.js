jQuery(document).ready(function($) {
    ...
    function AnimText(index){
        var text_anim='Хотите заказать корпоративный сайт? Ответьте на несколько вопросов по разработке уникального корпоративного сайта именно для ВАС!';
    ...
    function animateOn3() {
        var active = 1;
        if (!$('.pens').hasClass('a-check')) {
            function timer() {
                if (active > 6) {
                    $('.pen-f').removeClass('active');
                    active = 1;
                    $(".myChange" + active).addClass('active');
                }
                if (active <= 6) {
                    $('.pens').addClass('a-check');
                    $('.pen-f[data-id=' + active + ']').animate({
                        opacity: 0
                    }, 500);
                    $('.pen-i[data-id=' + active + ']').hide();
                    $('.pen-t[data-id=' + active + ']').animate({
                        opacity: 1
                    }, 500, function() {
                        setTimeout(function() {
                            $('.pen-i[data-id=' + active + ']').show();
                        }, 200);
                        $('.pen-i[data-id=' + active + ']').css('filter', 'invert(0)').promise().done(function() {
                            if (active % 2 != 0) {
                                $('.pen-t[data-id=' + active + ']').animate({
                                    left: '65%'
                                }, 500, function() {
                                    $('.pen-text[data-id=' + active + ']').css('border', '1px solid #60ff00');
                                    setTimeout(function() {
                                        anim2();
                                    }, 1000);
                                });
                ...
                function anim2() {
                    $('.pen-t[data-id=' + active + ']').animate({
                        ...
                        $('.pen-text').css('border', '1px solid transparent');
                        $('.pen-i[data-id=' + active + ']').css('filter', 'invert(0.5)');
                        $('.pen-f').removeClass('active');
                        active++;
                        $(".myChange" + active).addClass('active');
                    });
        ...
        setInterval(function() {
            if (luch_3 < 3) {
                $('#comluch' + luch_3 + ' img').animate({
                    opacity: 1
                }, 1000);
                luch_3++;
            }
        }, 1000);
        $("#comluch1").css("-webkit-animation", "leaves 2s ease-in-out infinite alternate", "animation", "leaves 2s ease-in-out infinite alternate")
        var left_luch = 1;
        setInterval(function() {
            if (left_luch < 3) {
    ...
    if (!firstAnimate && hash == '#structure' || hash == '#portfolio' || hash == '#multilanding' || hash == '#function') {
        nextSection();
    }
